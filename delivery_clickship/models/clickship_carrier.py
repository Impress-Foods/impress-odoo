...
class ClickShipCarrier(models.Model):
    ...
    def clickship_rate_shipment(
        self, order: StockPicking | SaleOrder
    ) -> dict[str, bool | float]:
    ...
    def clickship_get_raw_rates(self, order: StockPicking | SaleOrder) -> RateResponse:
        contact = self.clickship_contact
        sr = ClickshipProvider(
            self.log_xml,
            self.env,
    ...
    def clickship_send_shipping(self, pickings: StockPicking) -> list[dict[str, Any]]:
        contact = self.clickship_contact
        sr = ClickshipProvider(
            self.log_xml,
            self.env,
    ...
    def clickship_get_tracking_link(self, picking: StockPicking) -> str:
        return picking.clickship_tracking_url

    def clickship_cancel_shipment(self, picking: StockPicking) -> None:
        sr = ClickshipProvider(
            self.log_xml,
            self.env,
            prod_environment=self.prod_environment,
        ...
        if res:
            ...
            raise ValidationError(self.env._("Failed to cancel shipment!"))
    ...
    def button_get_payment_methods(self) -> None:
        ...
        if not self.clickship_api_key:
            raise ValidationError(self.env._("Clickship API key is not set."))
